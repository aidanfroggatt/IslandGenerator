...
public class ElevationHeatmap implements Buildable {
    @Override
    public Mesh build(Mesh aMesh) {
        int min = Integer.MAX_VALUE;
        int max = Integer.MIN_VALUE;
        for (Polygon polygon : aMesh.getPolygons()) {
            if (polygon.getAltitude() < min) {
                min = polygon.getAltitude();
            }
            if (polygon.getAltitude() > max) {
                max = polygon.getAltitude();
            }
        }
        ...
        for (Polygon polygon : aMesh.getPolygons()) {
            ...
            color = (int) Math.ceil(divisions * polygon.getAltitude());
